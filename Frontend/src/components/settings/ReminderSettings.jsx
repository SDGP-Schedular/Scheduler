...
const ReminderSettings = ({ isModal = false, onClose }) => {
    ...
    const [isPushSupported, setIsPushSupported] = useState(isPushNotificationSupported());
    ...
    const dailyTimeOptions = buildDailyTimeOptions();
    ...
    // Initialize notifications when component mounts and user is authenticated
    useEffect(() => {
        if (auth.currentUser && isPushSupported) {
            initializeNotifications((toastMsg) => {
                setToast(toastMsg);
            }).catch(error => {
    ...
    // Handler for push notification toggle with permission request
    const handlePushNotificationToggle = async (newValue) => {
        if (newValue && isPushSupported) {
            ...
            if (token) {
                ...
                updateSetting('preferences', 'push', true);
                setToast({ message: 'Push notifications enabled!', type: 'success' });
            } else {
                // Permission denied
                setToast({ 
                    message: 'Push notification permission denied. Check your browser settings.', 
                    type: 'warning' 
                });
                // Don't update settings since permission was denied
            }
        } else if (!newValue) {
            // User is disabling push notifications
            updateSetting('preferences', 'push', false);
                    ...
                    {settings.dailyReminders.enabled && (
                        <div className="setting-controls">
                            <div className="select-wrapper">
                                ...
                                >
                                    {dailyTimeOptions.map((timeOption) => (
                    ...
                    <div className="pref-list">
                        ...
                        <div className="pref-item">
                            ...
                            <div
                                ...
                                onClick={() => handlePushNotificationToggle(!settings.preferences.push)}
                            ></div>
                        </div>
                        <div className="pref-item">
                            <div className="pref-label">
